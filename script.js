...
const progress=document.querySelector('.scroll-progress'),header=document.querySelector('.site-header');let lastY=0;function onScroll(){const max=document.documentElement.scrollHeight-innerHeight;progress.style.width=`${max>0?(scrollY/max)*100:0}%`;header.classList.toggle('scrolled',scrollY>35);header.classList.toggle('hide',scrollY>180&&scrollY>lastY);lastY=scrollY}addEventListener('scroll',onScroll,{passive:true});onScroll();
